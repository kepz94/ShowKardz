...
describe('packedCards', () => {
  ...
  it('is empty when nothing is packed', () => {
    expect(packedCards(db({ ...base, packedStackIds: [] }))).toEqual([]);
  });

  it('returns only the cards in the packed groups', () => {
    const got = packedCards(db({ ...base, packedStackIds: ['g1'] }));
  ...
  it('can pack the loose cards, because they travel too', () => {
    ...
    const got = packedCards(db({ ...base, packedStackIds: [''] }));
  ...
  it('ignores a packed id whose group no longer exists', () => {
    ...
    const got = packedCards(db({ ...base, packedStackIds: ['g1', 'gone'] }));
    expect(got.map((c) => c.id).sort()).toEqual(['1', '2']);
  });
});
...
describe('packedSummary', () => {
  ...
  it('reports nothing packed as zeros and no names', () => {
    expect(packedSummary(db({ ...base, packedStackIds: [] })))
  ...
  it('counts unpriced cards INSIDE the case, not across the whole book', () => {
    ...
    const withOutsider = db({
      ...base,
      cards: [...base.cards, card('5', 'g2', 'unpriced')],
      packedStackIds: ['g1'],
    });
    expect(packedSummary(withOutsider).unpricedCount).toBe(1);
  });

  it('names the loose bucket so the summary is readable', () => {
    const loose = db({
      ...base,
      cards: [...base.cards, card('6', undefined, 'available', 100)],
      packedStackIds: [''],
    });
